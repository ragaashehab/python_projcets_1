...
connector.execute(
"CREATE TABLE IF NOT EXISTS STUDENT_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT,  TEXT, STREAM TEXT)"  #DOB
)
...
#creating StringVar or IntVar variables
...
def reset_fields():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, stream_strvar  #, dob
...
def display_records():
    ...
    data = c.fetchall()  # fetches all the data and stores it in the data variable.
    for records in data:
...
def add_record():
    ...
    if not name or not email or not contact or not gender  or not stream:   #or not DOB
        mb.showerror('Error!', "Please enter all the details!")
    else:
        try:
            connector.execute(
                'INSERT INTO Student_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, STREAM) VALUES (?,?,?,?,?)',
                (name, email, contact, gender, stream)
            )
        ...
        #    )
            ...
            mb.showerror('Wrong type', 'missed data, The contact number should be 10 digits')
